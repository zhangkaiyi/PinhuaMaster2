...
{
    public partial class EditModal_采购入库D : ComponentBase
    {
        [Parameter] public EventCallback<EditModal_采购入库D> OnOK { get; set; }
        [Parameter] public EventCallback<EditModal_采购入库D> OnCancel { get; set; }

        protected KModal modal;
        protected string[] dropdownItems => new[] { "片", "平方米", "延长米", "立方米", "套" };

        protected dto采购入库D currentDataSource
        {
            get
            {
                return DataSource;
        ...
        [Parameter] public dto采购入库D DataSource { get; set; }
