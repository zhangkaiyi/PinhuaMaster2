...
{
    public partial class Modal_商品列表_采购询价 : ComponentBase
    {
        protected KModal modal;
        protected KTable2 table;
        protected List<dto采购询价D> products;

        [Inject] IMapper Mapper { get; set; }
        [Inject] Pinhua2Context PinhuaContext { get; set; }

        [Parameter] public EventCallback<Modal_商品列表_采购询价> OnOK { get; set; }
        [Parameter] public EventCallback<Modal_商品列表_采购询价> OnCancel { get; set; }

        protected override void OnInitialized()
        {
            products = Mapper.ProjectTo<dto采购询价D>(PinhuaContext.tb_报价表D).ToList();
