...
{
    public partial class Modal_商品列表_采购申请 : ComponentBase
    {
        protected KModal modal;
        protected KTable2 table;
        protected List<dto采购申请D> products;

        [Inject] IMapper Mapper { get; set; }
        [Inject] Pinhua2Context PinhuaContext { get; set; }

        [Parameter] public EventCallback<Modal_商品列表_采购申请> OnOK { get; set; }
        [Parameter] public EventCallback<Modal_商品列表_采购申请> OnCancel { get; set; }

        protected override void OnInitialized()
        {
            products = Mapper.ProjectTo<dto采购申请D>(PinhuaContext.tb_需求表D).ToList();
