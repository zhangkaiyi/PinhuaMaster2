...
namespace Piuhua2.Components.Modal
{
    public partial class Modal_商品列表_销售报价D : ComponentBase
    {
        ...
        protected List<combo销售报价> Combo;

        protected List<dto销售报价D> DataSource;
        protected List<dto销售报价D> currentDataSource
        {
            get
            {
                ...
                {
                    return Mapper.Map<IEnumerable<dto销售报价D>>(Combo.Where(FilterExpression.Compile()).Select(d => d.Detail)).ToList();
                }
                else
                {
                    return Mapper.Map<IEnumerable<dto销售报价D>>(Combo.Select(d=>d.Detail)).ToList();
                }
            }
        }
        ...
        {
            Combo = PinhuaContext.GetViews().销售.销售报价combo();
